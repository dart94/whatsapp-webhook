...
import type { RecipientData, SheetRowData } from "@/types/sheet";
...
export default function EnviarPlantillaPage() {
  ...
  // Estados principales
  const [selectedSheet, setSelectedSheet] = useState<RegisteredSheet | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const processedRecipients: RecipientData[] = sheetData.map(
      (row: SheetRowData, index: number) => {
  ...
  useEffect(() => {
    ...
    const loadSheetData = async () => {
      try {
        ...
        const processedRecipients: RecipientData[] = data.map(
          (row: SheetRowData, index: number) => {
            const phone = String(
              row.Celular || row.Whatsapp || row.Telefono || ""
            );
            const name = String(
              row.Nombre || row.Name || `Contacto ${index + 1}`
            );

            // Extraer variables automáticamente según los headers
            const variables = Array(variableCount)
              .fill("")
              .map((_, i) => {
    ...
    setRecipients((prev) =>
      prev.map((r) =>
        r.id === recipientId
          ? {
              ...r,
              variables: r.variables.map((v, i) => (i === index ? value : v)),
            }
          : r
      )
    );
  ...
  const handleBulkSend = async () => {
    ...
    for (let i = 0; i < selectedRecipientsList.length; i++) {
      ...
      try {
        ...
        setRecipients((prev) =>
          prev.map((r) =>
            r.id === recipient.id ? { ...r, status: "error" } : r
          )
        );
      }
  ...
  if (loadingTemplates) {
    return <Loader message="Cargando plantillas" showTips={true} />;
  }
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center space-x-4">
            <div className="text-sm text-gray-600">
              {selectedCount} seleccionados de {validRecipients.length}{" "}
              disponibles
            </div>
          </div>
        </div>
