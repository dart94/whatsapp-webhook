          ...
          {initialized && loading ? (
            ...
            <>
              {filteredNavigationItems.map((item) => (
                <NavLink key={item.href} {...item} onClick={handleMobileClick} />
              ))}
              ...
              <div className="space-y-1">
                ...
                <div
                  className={`
                    ...
                    ${openTemplates ? "max-h-28 opacity-100" : "max-h-0 opacity-0"}
                  `}
                >
        ...
        <div className="p-4 border-t border-purple-700/60 shrink-0 space-y-3">
          {authSnap.hasUser ? (
            <div className="text-sm text-gray-200 mb-2">
              ...
              {authSnap.isAdmin && (
                <div className="text-xs text-yellow-300 font-medium">👑 Admin</div>
              )}
            </div>
          ) : (
            ...
            onClick={() => {
              showSweetAlert({
                ...
                if (result.isConfirmed) {
                  // Cerrar sesión desde el contexto
                  // (tu logout ya limpia storage y redirige)
                  if (isMobile && onToggle) onToggle();
                  // Llamar logout
                  // Nota: lo dejo al final para evitar cortar el SweetAlert/animación
                  setTimeout(() => logout(), 0);
                }
              });
            }}
  ...
  return (
    <Link
      ...
      className={`
        ...
        transition-colors duration-200 group
        ${active ? "bg-purple-200 text-black font-medium" : "text-gray-200 hover:bg-purple-300 hover:text-white"}
      `}
    >
  ...
  return (
    <Link
      ...
      className={`
        ...
        ${active ? "bg-purple-200 text-black font-medium" : "text-gray-300 hover:bg-purple-300 hover:text-white"}
      `}
    >
